...
public class Article implements Serializable {
    ...
    public Article info() {
        if (Objects.nonNull(this.categoryId)) {
            ...
            Category category = categoryDao.getById(this.categoryId);
            setCategoryName(category.getName());
        }
        TagDao tagDao = SpringContextHolder.getBean(TagDao.class);
        ArticleTagDao articleTagDao = SpringContextHolder.getBean(ArticleTagDao.class);

        Set<BigInteger> tagIdSet = articleTagDao.lambdaQuery()
                .eq(ArticleTag::getArticleId, this.id)
                .list()
                .stream().map(ArticleTag::getTagId)
                .collect(Collectors.toSet());

        Set<String> tagNameSet = tagDao.listByIds(tagIdSet)
                .stream()
                .map(Tag::getName)
                .collect(Collectors.toSet());
        setTagNameList(tagNameSet.stream().toList());
